module Syscall::Handlers
  ...
  def sbrk(args : Syscall::Arguments)
    ...
    if (incr & 0xfff != 0) || args.process.udata.mmap_heap.nil?
      return 0
    end
    mmap_heap = args.process.udata.mmap_heap.not_nil!
    if incr > 0
  ...
  def mmap(args : Syscall::Arguments)
    ...
    prot = Syscall::Data::MmapProt.new(args[1].to_i32)
    ...
    if fdi == -1
      ...
      Paging.alloc_page addr,
        ...
        true, size // 0x1000
      args.process.udata.mmap_list.add(addr, size, mmap_attrs)
      addr
    else
      mmap_attrs |= MemMapList::Node::Attributes::SharedMem
      fd = args.process.udata.get_fd(fdi) || return 0
      ...
      mmap_node = args.process.udata.mmap_list.space_for_mmap args.process, size, mmap_attrs
      if mmap_node
        if fd.node.not_nil!.mmap(mmap_node, args.process) == VFS_OK
          mmap_node.shm_node = fd.node
          return mmap_node.addr
        else
          args.process.udata.mmap_list.remove mmap_node
          0
        end
      else
        0
  ...
  def munmap(args : Syscall::Arguments)
    ...
    args.process.udata.mmap_list.each do |node|
      if node.addr == addr && (size == node.size || full_size)
        if node.attr.includes?(MemMapList::Node::Attributes::SharedMem)
          node.shm_node.not_nil!.munmap(node.addr, node.size, args.process)
        else
          i = 0
          while i < node.size
        ...
        args.process.udata.mmap_list.remove(node)
        return 0
        ...
        args.process.udata.mmap_list.split_node(node, addr, size)
        return 0
      end
    end
    EINVAL
